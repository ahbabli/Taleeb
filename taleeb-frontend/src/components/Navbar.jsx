...
{ key: "class-feed", label: "Class", icon: ClipboardList },
...
...studentLinks,
{ key: "manage-class-posts", label: "Manage", icon: ClipboardList },
];


const adminLinks = [
  { key: "admin-analytics", label: "Stats", icon: BarChart3 },
...
return (
<div
    className="fixed bottom-3 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-1rem)] max-w-xl sm:bottom-4 sm:w-[calc(100%-2rem)]">
    <div
        className="bg-white/95 backdrop-blur-xl border border-blue-100 shadow-2xl rounded-[1.5rem] px-1.5 py-2 flex items-center justify-between gap-1 sm:rounded-[2rem] sm:px-3 sm:py-3">
        {links.map((link) => {
        const Icon = link.icon;
        const active = currentPage === link.key;

        return (
        <button key={link.key} onClick={()=> setCurrentPage(link.key)}
            className={`flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 px-1 py-2 rounded-2xl
            transition-all sm:gap-1 sm:px-3 ${
            active
